...
public class LongStackTest {
   ...
   @Test(timeout=1000)
   public void testInterpretSwapAndRot() {
      String s = "2 5 9 ROT + SWAP -";
      assertEquals("expression: " + Aout.toString (s), 6,
              LongStack.interpret (s));
   }

   @Test(expected=RuntimeException.class)
   public void testInterpretSwapFail() {
      String s = "1 SWAP";
      LongStack.interpret (s);
   }

   @Test(expected=RuntimeException.class)
   public void testInterpretRotFail() {
      String s = "3 4 ROT";
      LongStack.interpret (s);
   }
